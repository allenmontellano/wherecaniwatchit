...
- **SP6 (Auth & roles, invite-only) — BUILD COMPLETE on `feat/sp6-auth`, PAUSED AT STAGING GATE (2026-06-16).** Spec `docs/superpowers/specs/2026-06-16-sp6-auth-roles-design.md`, plan `docs/superpowers/plans/2026-06-16-sp6-auth-roles.md`. All 9 impl tasks built TDD via subagents with spec+quality review each; final branch review = **READY FOR STAGING**. `npx tsc --noEmit` clean, **259 tests green**, eslint clean.
  ...
  - **STAGING GATE (2026-06-16): MERGED + VERIFIED — but invite-accept is BROKEN; PROD HELD.** `feat/sp6-auth` merged → `staging` (`af9c485`, pushed); deploy live at staging.wherecaniwatchit.info. Migration applied+verified on staging; **prod `ahgmszdrhndcycvairmn` untouched**. Token was rotated by user. **Live staging verification results:**
    ...
    - ❌ **BLOCKER — invite onboarding broken.** Supabase invite email link uses the **implicit/hash flow**: verify endpoint 303-redirects to `/accept-invite#access_token=…`. Tokens live in the URL **hash** (client-only); the server never sees them, **no app session cookie is set**, and there is **no `/auth/confirm` route** and no client-side hash handler on `/accept-invite`. So the accept-invite server action's `getUser()` returns null → "invite link expired." An invited user cannot complete onboarding. *(Root cause: SSR needs a code/token-hash exchange step the spec/plan missed; mocked unit tests + code-only review couldn't catch it — the live gate did.)*
    - **FIX OPTIONS (awaiting user decision):** (A, recommended) add `app/auth/confirm/route.ts` GET handler calling `supabase.auth.verifyOtp({type,token_hash})` to set cookies then redirect to `/accept-invite`, + change the Supabase "Invite user" email template to a `{{ .TokenHash }}` link to `/auth/confirm?...&next=/accept-invite` (dashboard config). (B) handle the hash client-side on `/accept-invite` (mount a browser client so `detectSessionInUrl` writes cookies) — no dashboard change, but tokens transit the URL hash.
    - **NEXT:** fix invite-accept (small addition to SP6), re-verify the full invite→accept on staging, THEN separate prod approval. Do not ship prod until invite onboarding works.
