...
```
(behind the scenes|making of|paleyfest|featurette|deleted scene|bloopers?|gag reel|\mextras?\M|\mclips?\M|sneak peek|first look|red carpet|press conference|panel)
```

- `clip`/`clips` and `extra`/`extras` are word-boundaried (`\m…\M`) so "Extraction" or "Eclipse" don't match.
